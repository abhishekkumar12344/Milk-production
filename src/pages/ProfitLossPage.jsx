            ...
            {EXPENSES.map(e => (
              <div key={e.id}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: isMobile ? 12 : 13, marginBottom: 4 }}>
                  <span style={{ fontWeight: 600, wordBreak: "break-word" }}>{e.category}</span>
                  <span style={{ fontWeight: 700, color: COLORS.danger, whiteSpace: "nowrap", marginLeft: 8 }}>{formatCurrency(e.amount)}</span>
                </div>
                ...
                <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>{e.note}</div>
              </div>
            ))}
          </div>
      ...
      <div style={s.grid(3)}>
        {[
          ...
          <div key={title} style={s.card}>
            <div style={{ fontSize: isMobile ? 14 : 16, fontWeight: 700, marginBottom: 4 }}>{title}</div>
            <div style={{ fontSize: isMobile ? 11 : 12, color: "#94a3b8", marginBottom: 12 }}>{period}</div>
            <div style={{ fontSize: isMobile ? 12 : 14, color: COLORS.accent, fontWeight: 600, marginBottom: 4 }}>{r}</div>
            <div style={{ fontSize: isMobile ? 12 : 14, color: COLORS.primary, fontWeight: 600, marginBottom: 12 }}>{p}</div>
            <button style={{ ...s.btn(COLORS.primary, true), marginTop: 0, fontSize: isMobile ? 11 : 12, padding: "6px 12px", width: "100%" }}>📄 Download PDF</button>
          </div>
        ))}
      </div>
    </div>
