...
export default function HomePage({ onLogin }) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#f0f6ff", minHeight: "100vh" }}>
      {/* NAVBAR */}
      <nav style={{ position: "sticky", top: 0, background: scrolled ? "rgba(255,255,255,0.95)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", padding: "14px 48px", display: "flex", alignItems: "center", justifyContent: "space-between", zIndex: 100, transition: "all 0.3s", boxShadow: scrolled ? "0 2px 12px rgba(0,0,0,0.08)" : "none" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontWeight: 800, fontSize: 18, color: "#1a56db" }}>DairyFlow</span>
        </div>
        ...
        <button onClick={onLogin} style={{ padding: "10px 24px", background: COLORS.primary, color: "white", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
          Launch App →
        </button>
      </nav>

      {/* HERO */}
      <section style={{ padding: "80px 48px 60px", textAlign: "center", background: "linear-gradient(135deg, #eef2ff 0%, #f0fdf4 100%)" }}>
        <div style={{ display: "inline-block", padding: "6px 16px", background: "#dbeafe", borderRadius: 20, color: "#1a56db", fontSize: 12, fontWeight: 700, marginBottom: 20, letterSpacing: "0.05em" }}>🆕 SMART DAIRY MANAGEMENT</div>
        <h1 style={{ fontSize: 56, fontWeight: 900, color: "#0f172a", lineHeight: 1.1, margin: "0 auto 20px", maxWidth: 720 }}>
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={onLogin} style={{ padding: "14px 32px", background: COLORS.primary, color: "white", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer", boxShadow: "0 8px 24px rgba(26,86,219,0.35)" }}>
            Start Free Trial 🚀
          </button>
          <button style={{ padding: "14px 32px", background: "white", color: COLORS.primary, border: `2px solid ${COLORS.primary}`, borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer" }}>
            Watch Demo ▶
          </button>
        </div>
      ...
      <section style={{ padding: "72px 48px", background: "white" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <h2 style={{ fontSize: 36, fontWeight: 800, color: "#0f172a", margin: "0 0 12px" }}>Everything You Need</h2>
          <p style={{ color: "#64748b", fontSize: 16 }}>A complete system built for village dairy business operations</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, maxWidth: 1100, margin: "0 auto" }}>
          {features.map((f) => (
            <div key={f.title} style={{ padding: 28, border: "1.5px solid #e2e8f0", borderRadius: 16, transition: "all 0.2s" }}>
              <div style={{ fontSize: 36, marginBottom: 14 }}>{f.icon}</div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: "#1e293b", margin: "0 0 8px" }}>{f.title}</h3>
      ...
      <section style={{ padding: "72px 48px", background: "#f8fafc" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <h2 style={{ fontSize: 36, fontWeight: 800, color: "#0f172a", margin: "0 0 12px" }}>Powerful Analytics</h2>
          <p style={{ color: "#64748b", fontSize: 16 }}>Visual insights to grow your dairy business</p>
        </div>
        <div style={{ background: "white", borderRadius: 20, padding: 28, maxWidth: 900, margin: "0 auto", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          <div style={{ marginBottom: 16, fontWeight: 700, color: "#1e293b" }}>Monthly Milk Collection & Revenue (Liters)</div>
      ...
      <section style={{ padding: "72px 48px", background: "white" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <h2 style={{ fontSize: 36, fontWeight: 800, color: "#0f172a", margin: "0 0 12px" }}>Trusted by Dairy Owners</h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, maxWidth: 960, margin: "0 auto" }}>
          {testimonials.map((t) => (
            <div key={t.name} style={{ padding: 28, border: "1.5px solid #e2e8f0", borderRadius: 16 }}>
      ...
      {/* CTA */}
      <section style={{ padding: "72px 48px", background: "linear-gradient(135deg, #1a56db, #10b981)", textAlign: "center" }}>
        <h2 style={{ fontSize: 40, fontWeight: 900, color: "white", margin: "0 0 16px" }}>Ready to Digitize Your Dairy?</h2>
        <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 17, marginBottom: 32 }}>Join hundreds of dairy owners already saving time with DairyFlow</p>
        <button onClick={onLogin} style={{ padding: "14px 40px", background: "white", color: COLORS.primary, border: "none", borderRadius: 12, fontSize: 16, fontWeight: 800, cursor: "pointer" }}>
          Get Started Free →
        </button>
      </section>

      {/* FOOTER */}
      <footer style={{ background: "#0f172a", padding: "36px 48px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        <div>
