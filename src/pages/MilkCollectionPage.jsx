...
export default function MilkCollectionPage({ dark }) {
  ...
  const [collections, setCollections] = useState(
    Array.isArray(MILK_COLLECTIONS) ? MILK_COLLECTIONS : []
  );
  ...
  // Add New Entry
  const handleAdd = () => {
    if (!form.distributorName || !form.quantity) {
  ...
  // Day Stats
  const dayTotal = filtered.reduce(
    (a, b) => a + Number(b.quantity || 0),
    0
  );

  const dayAmt = filtered.reduce(
