 ...
 * Production-level Analytics & Reports page with:
 *  - Real-time data from backend API
 *  - Date-wise / custom range filtering
 *  - Milk Collection, Sales, Distribution, P&L, Production analysis
 *  - CSV download for every data section
 *  - PDF reports (profit-loss, inventory)
 *  - Receipt download per collection / payment / sale
 */
...
import {
  analyticsAPI, milkCollectionAPI, salesAPI,
  paymentAPI, reportsAPI, milkCollectionAPI as mcAPI,
  distributorAPI, clientAPI, productionAPI,
  reportsAPI as rAPI,
} from "../utils/api.js";
...
function Spinner({ color = COLORS.primary }) {
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 40 }}>
      <div style={{
        width: 36, height: 36, border: `3px solid ${color}20`,
...
function DateRangePicker({ startDate, endDate, onChange, dark }) {
  ...
  useEffect(() => {
    const fn = (e) => { if (ref.current && !ref.current.contains(e.target)) setShowPresets(false); };
    document.addEventListener("mousedown", fn);
    return () => document.removeEventListener("mousedown", fn);
  }, []);

  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center", position: "relative" }} ref={ref}>
        ...
        <div style={{
          ...
          {PRESET_RANGES.map((p) => (
            <button
              ...
              onMouseEnter={(e) => e.target.style.background = dark ? "rgba(255,255,255,0.07)" : "#f1f5f9"}
...
function OverviewTab({ dark, startDate, endDate }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([d, m, dy, p]) => {
        setDash(d.data); setMonthly(m.data || []); setDaily(dy.data || []); setPL(p.data);
      })
  ...
  if (error) return (
    <div style={{ padding: 24, textAlign: "center", color: COLORS.danger }}>
  ...
  const stats = [
    ...
    { icon: "🏆", label: "Net Profit", value: dash.netProfit?.value, color: dash.netProfit?.raw >= 0 ? COLORS.accent : COLORS.danger, positive: dash.netProfit?.raw >= 0 },
  ...
  const CHART_COLORS = [COLORS.primary, COLORS.accent, COLORS.warning, COLORS.danger, COLORS.purple];

  // Prepare daily chart: last 14 days
  ...
  const plData = pl ? [
    ...
    { name: "Net Profit", value: pl.netProfit || 0, fill: pl.netProfit >= 0 ? COLORS.purple : COLORS.danger },
  ] : [];
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <div style={s.card}>
          <SectionHeader title="Monthly Revenue vs Profit (12 months)" dark={dark}
            action={
              <button onClick={() => downloadCSV("monthly-trend.csv", monthly, [
          ...
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={monthly}>
              ...
              <YAxis tick={{ fontSize: 11, fill: dark ? "#94a3b8" : "#64748b" }} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}k`} />
        ...
        <div style={s.card}>
          <SectionHeader title="Daily Collection & Sales (Last 14 Days)" dark={dark}
            action={
              <button onClick={() => downloadCSV("daily-14.csv", recentDaily, [
                { label: "Date", key: "date" },
      ...
      {/* P&L summary for selected range */}
      {pl && (
        <div style={s.card}>
          <SectionHeader title={`P&L Summary: ${fmtDate(pl.period?.start)} → ${fmtDate(pl.period?.end)}`} dark={dark}
            action={
              <button
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12, marginBottom: 16 }}>
            {plData.map((item) => (
              <div key={item.name} style={{
                ...
                <div style={{ fontSize: 12, color: dark ? "#94a3b8" : "#64748b", fontWeight: 600, marginBottom: 4 }}>{item.name}</div>
                <div style={{ fontSize: 22, fontWeight: 800, color: item.fill }}>{formatCurrency(item.value)}</div>
              </div>
            ))}
            <div style={{
              padding: "16px 18px", borderRadius: 12,
              background: (pl.profitMargin >= 0 ? COLORS.accent : COLORS.danger) + "12",
              border: `1.5px solid ${(pl.profitMargin >= 0 ? COLORS.accent : COLORS.danger)}30`,
            }}>
              <div style={{ fontSize: 12, color: dark ? "#94a3b8" : "#64748b", fontWeight: 600, marginBottom: 4 }}>Profit Margin</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: pl.profitMargin >= 0 ? COLORS.accent : COLORS.danger }}>{pl.profitMargin}%</div>
            </div>
          </div>
          ...
          {pl.expenses?.breakdown && Object.keys(pl.expenses.breakdown).length > 0 && (
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: dark ? "#94a3b8" : "#64748b", marginBottom: 12 }}>Expense Breakdown</div>
                  ...
                  return (
                    <div key={cat}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 3 }}>
                        ...
                        <span style={{ color: COLORS.danger, fontWeight: 700 }}>{formatCurrency(amt)} ({pct.toFixed(1)}%)</span>
                      </div>
                      <div style={{ height: 6, background: dark ? "rgba(255,255,255,0.06)" : "#e2e8f0", borderRadius: 4 }}>
                        <div style={{ width: `${pct}%`, height: "100%", background: COLORS.danger, borderRadius: 4, transition: "width 0.4s" }} />
                      </div>
                    </div>
                  );
                })}
...
function CollectionTab({ dark, startDate, endDate }) {
  ...
  // filter by date range + search + shift
  const filtered = data.filter((c) => {
    ...
    if (search && !c.distributorName?.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });

  const totalLiters = filtered.reduce((s, c) => s + (c.quantity || 0), 0);
  const totalRevenue = filtered.reduce((s, c) => s + (c.total || 0), 0);
  const avgFat = filtered.length ? (filtered.reduce((s, c) => s + (c.fat || 0), 0) / filtered.length).toFixed(2) : 0;

  // daily chart
  const dailyMap = {};
  filtered.forEach((c) => {
    if (!dailyMap[c.date]) dailyMap[c.date] = { date: c.date, quantity: 0, revenue: 0 };
    dailyMap[c.date].quantity += c.quantity || 0;
    dailyMap[c.date].revenue += c.total || 0;
  });
  const dailyChart = Object.values(dailyMap).sort((a, b) => a.date.localeCompare(b.date)).slice(-30);
      ...
      <div style={{ ...s.card, marginBottom: 16 }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={dailyChart}>
            ...
            <Tooltip formatter={(v, n) => n === "revenue" ? formatCurrency(v) : `${v} L`} contentStyle={{ background: dark ? "#1e293b" : "white", border: "none", borderRadius: 8 }} />
            <Legend />
      ...
      <div style={s.card}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign: "center", padding: 32, color: "#94a3b8" }}>No collections in selected range</td></tr>
              ) : (
                filtered.map((c) => (
                  <tr key={c._id}>
                    <td style={s.td}>{fmtDate(c.date)}</td>
                    <td style={{ ...s.td, fontWeight: 600 }}>{c.distributorName}</td>
                    <td style={s.td}>
                      <span style={{ ...s.badge2(c.shift === "Morning" ? COLORS.warning : COLORS.purple) }}>{c.shift}</span>
                    </td>
                    ...
                    <td style={s.td}>
                      <span style={s.badge2(c.status === "Paid" ? COLORS.accent : COLORS.warning)}>{c.status}</span>
                    </td>
                    <td style={s.td}>
                      <button
                        onClick={() => milkCollectionAPI.downloadReceipt(c._id).catch((e) => alert("Receipt: " + e.message))}
...
function SalesTab({ dark, startDate, endDate }) {
  ...
  const filtered = data.filter((s) => {
    ...
    const name = s.clientId?.name || s.clientName || "";
    if (search && !name.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });

  const totalRev = filtered.reduce((a, b) => a + (b.total || 0), 0);
  const paidRev = filtered.filter((x) => x.paymentStatus === "Paid").reduce((a, b) => a + (b.total || 0), 0);
  const pendingRev = filtered.filter((x) => x.paymentStatus !== "Paid").reduce((a, b) => a + (b.total || 0), 0);

  // Group by product
  const byProduct = {};
  filtered.forEach((s) => {
    const k = s.productId?.item || s.product || "Other";
    if (!byProduct[k]) byProduct[k] = { name: k, total: 0, qty: 0 };
    byProduct[k].total += s.total || 0;
    byProduct[k].qty += s.quantity || 0;
  });
  const productChart = Object.values(byProduct).sort((a, b) => b.total - a.total).slice(0, 8);
  const PIE_COLORS = [COLORS.primary, COLORS.accent, COLORS.warning, COLORS.purple, COLORS.danger, "#06b6d4", "#ec4899", "#84cc16"];
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 14, marginBottom: 20 }}>
        ...
        <StatCard icon="✅" label="Paid" value={formatCurrency(paidRev)} color={COLORS.primary} dark={dark} positive />
        <StatCard icon="⏳" label="Pending" value={formatCurrency(pendingRev)} color={COLORS.warning} dark={dark} />
        <StatCard icon="📋" label="Invoices" value={filtered.length} color={COLORS.purple} dark={dark} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={productChart} layout="vertical">
              ...
              <Bar dataKey="total" radius={[0, 6, 6, 0]}>
                {productChart.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={s.card}>
          <SectionHeader title="Revenue Split" dark={dark} />
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={[
      ...
      <div style={s.card}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16, justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <input placeholder="Search client..." value={search} onChange={(e) => setSearch(e.target.value)}
              style={{ ...s.input, width: 200, padding: "7px 12px" }} />
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ ...s.select, width: 140 }}>
              <option value="">All Status</option>
              <option>Paid</option>
              <option>Pending</option>
            </select>
          </div>
          <button onClick={() => downloadCSV(`sales-${startDate}-${endDate}.csv`, filtered, csvCols)}
            style={{ ...s.btn(COLORS.accent, false), padding: "7px 16px" }}>
            ⬇ Download CSV ({filtered.length})
          </button>
        </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["Date", "Client", "Product", "Qty", "Total", "Status", "Invoice"].map((h) => (
                  <th key={h} style={s.th}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: "center", padding: 32, color: "#94a3b8" }}>No sales in selected range</td></tr>
              ) : (
                filtered.map((sale) => (
                  <tr key={sale._id}>
                    <td style={s.td}>{fmtDate(sale.date)}</td>
                    <td style={{ ...s.td, fontWeight: 600 }}>{sale.clientId?.name || sale.clientName || "-"}</td>
                    <td style={s.td}>{sale.productId?.item || sale.product || "-"}</td>
                    <td style={s.td}>{sale.quantity} {sale.productId?.unit || sale.unit || ""}</td>
                    <td style={{ ...s.td, fontWeight: 700, color: COLORS.accent }}>{formatCurrency(sale.total)}</td>
                    <td style={s.td}>
                      <span style={s.badge2(sale.paymentStatus === "Paid" ? COLORS.accent : COLORS.warning)}>{sale.paymentStatus}</span>
                    </td>
                    <td style={s.td}>
                      <button
                        onClick={() => salesAPI.downloadInvoice(sale._id, sale.referenceNo).catch((e) => alert("Invoice: " + e.message))}
                        style={{ ...s.btn(COLORS.primary, true), padding: "4px 10px", fontSize: 11 }}
                      >
                        📄 Invoice
                      </button>
...
function ProfitLossTab({ dark, startDate, endDate }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([p, m]) => { setPL(p.data); setMonthly(m.data || []); })
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  }, [startDate, endDate]);
  ...
  if (pl.expenses?.distributorPayments) expData.push({ name: "Distributor Payments", value: pl.expenses.distributorPayments });

  const PIE_COLORS = [COLORS.primary, COLORS.accent, COLORS.warning, COLORS.danger, COLORS.purple, "#06b6d4"];
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 14, marginBottom: 20 }}>
        ...
        <StatCard icon="📈" label="Net Profit" value={formatCurrency(pl.netProfit)} color={pl.netProfit >= 0 ? COLORS.accent : COLORS.danger} dark={dark} positive={pl.netProfit >= 0} />
        <StatCard icon="📊" label="Profit Margin" value={`${pl.profitMargin}%`} color={pl.profitMargin >= 0 ? COLORS.accent : COLORS.danger} dark={dark} />
      </div>

      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16, gap: 10 }}>
        <button
          onClick={() => downloadCSV("profitloss.csv", monthly, [
            { label: "Month", key: "month" },
            { label: "Revenue", key: "revenue" },
            { label: "Expenses", key: "expenses" },
            { label: "Profit", key: "profit" },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <div style={s.card}>
          ...
          {expData.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={expData} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`}>
      ...
      <div style={s.card}>
        <SectionHeader title={`Detailed P&L: ${fmtDate(pl.period?.start)} to ${fmtDate(pl.period?.end)}`} dark={dark} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 6px", fontSize: 15, fontWeight: 800, borderTop: `1.5px solid ${dark ? "rgba(255,255,255,0.1)" : "#e2e8f0"}`, marginTop: 6 }}>
          ...
          <div>
            ...
            {pl.expenses?.distributorPayments > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", fontSize: 13 }}>
        ...
        <div style={{
          ...
          background: (pl.netProfit >= 0 ? COLORS.accent : COLORS.danger) + "12",
          border: `2px solid ${(pl.netProfit >= 0 ? COLORS.accent : COLORS.danger)}30`,
          display: "flex", justifyContent: "space-between", alignItems: "center",
        }}>
          <span style={{ fontSize: 16, fontWeight: 700 }}>NET {pl.netProfit >= 0 ? "PROFIT" : "LOSS"}</span>
          <span style={{ fontSize: 24, fontWeight: 900, color: pl.netProfit >= 0 ? COLORS.accent : COLORS.danger }}>
            {formatCurrency(Math.abs(pl.netProfit))}
          </span>
          <span style={{ fontSize: 14, fontWeight: 700, color: pl.netProfit >= 0 ? COLORS.accent : COLORS.danger }}>
...
function ProductionTab({ dark, startDate, endDate }) {
  ...
  useEffect(() => {
    setLoading(true); setError(null);
    productionAPI.getAll({ startDate, endDate })
      .then((r) => setData(r.data || []))
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  }, [startDate, endDate]);
    ...
    <div>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={prodChart} cx="50%" cy="50%" outerRadius={85} dataKey="count" nameKey="name"
                ...
                {prodChart.map((_, i) => <Cell key={i} fill={[COLORS.primary, COLORS.accent, COLORS.warning, COLORS.purple, COLORS.danger][i % 5]} />)}
              </Pie>
              <Tooltip contentStyle={{ background: dark ? "#1e293b" : "white", border: "none", borderRadius: 8 }} />
            </PieChart>
          </ResponsiveContainer>
      ...
      <div style={s.card}>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
          <button onClick={() => downloadCSV(`production-${startDate}-${endDate}.csv`, filtered, csvCols)}
            style={{ ...s.btn(COLORS.accent, false), padding: "7px 16px" }}>
            ⬇ Download CSV ({filtered.length})
          </button>
        </div>
...
function DistributorsTab({ dark, startDate, endDate }) {
  ...
  // merge collections filtered by date range per distributor
  const distMap = {};
  ...
  const totalLiters = rows.reduce((s, r) => s + r.filteredLiters, 0);
  const totalRevenue = rows.reduce((s, r) => s + r.filteredRevenue, 0);
  const totalPending = rows.reduce((s, r) => s + r.pendingAmount, 0);

  const chartData = rows.slice(0, 8).map((r) => ({ name: r.name?.split(" ")[0] || "-", liters: r.filteredLiters, revenue: r.filteredRevenue }));
  ...
  const csvCols = [
    ...
    { label: "Total Liters (All)", key: "totalLiters" },
    { label: "Total Amount (All)", key: "totalAmount" },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ ...s.card, marginBottom: 16 }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={chartData} layout="vertical">
            ...
            <Tooltip formatter={(v, n) => n === "liters" ? `${v} L` : formatCurrency(v)} contentStyle={{ background: dark ? "#1e293b" : "white", border: "none", borderRadius: 8 }} />
            <Legend />
      ...
      <div style={s.card}>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
          <button onClick={() => downloadCSV(`distributors-${startDate}-${endDate}.csv`, rows, csvCols)}
            style={{ ...s.btn(COLORS.accent, false), padding: "7px 16px" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["Distributor", "Village", "Status", "Liters (Period)", "Revenue (Period)", "Pending", "Total Liters"].map((h) => (
                  <th key={h} style={s.th}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((d) => (
                <tr key={d._id}>
                  ...
                  <td style={s.td}><span style={s.badge2(d.status === "Active" ? COLORS.accent : COLORS.danger)}>{d.status}</span></td>
                  <td style={{ ...s.td, fontWeight: 700 }}>{formatNum(d.filteredLiters.toFixed(1))} L</td>
                  <td style={{ ...s.td, fontWeight: 700, color: COLORS.accent }}>{formatCurrency(d.filteredRevenue)}</td>
                  <td style={{ ...s.td, color: d.pendingAmount > 0 ? COLORS.danger : COLORS.accent, fontWeight: 700 }}>{formatCurrency(d.pendingAmount)}</td>
                  <td style={s.td}>{formatNum(d.totalLiters?.toFixed(1) || 0)} L</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function ReportsAnalyticsPage({ dark }) {
  ...
  const handleRangeChange = (s, e) => { setStartDate(s); setEndDate(e); };
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: dark ? "#e2e8f0" : "#1e293b" }}>📅 Date Range:</span>
          <span style={{ fontSize: 13, color: "#94a3b8" }}>{fmtDate(startDate)} → {fmtDate(endDate)}</span>
        </div>
        <DateRangePicker startDate={startDate} endDate={endDate} onChange={handleRangeChange} dark={dark} />
      </div>

      {/* Tabs */}
      <div style={{
        display: "flex", overflowX: "auto", gap: 4, marginBottom: 20,
        padding: "4px", background: dark ? "rgba(255,255,255,0.05)" : "rgba(26,86,219,0.05)",
